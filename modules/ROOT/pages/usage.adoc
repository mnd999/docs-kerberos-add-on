...
This is an example implementation using C# with the 5.18.0 .NET driver.

[source, csharp]
----
await using var driver = GraphDatabase.Driver(new Uri("bolt://neo4j.windomain.local:7687"), new KerberosTokenManager("neo4j"));
		
try	{
    var (result, _) = await driver
        .ExecutableQuery("MATCH () RETURN count(*) AS count")
        .WithMap(r => r["count"].As<int>())
        .WithReduce(() => 0, (a, b) => b)
        .ExecuteAsync();

     Console.WriteLine($"Node count: {result}");
}
catch (Exception e) {
	Console.WriteLine($"Error: {e.Message}");
}

class KerberosTokenManager(string serviceName) : IAuthTokenManager
{
    public ValueTask<IAuthToken> GetTokenAsync(CancellationToken cancellationToken = default)
    {
        return new ValueTask<IAuthToken>(AuthTokens.Kerberos(getTicket(serviceName)));
    }

    public ValueTask<bool> HandleSecurityExceptionAsync(IAuthToken token, SecurityException exception, CancellationToken cancellationToken = default)
    {
        throw new NotImplementedException();
    }

    static String getTicket(string serviceName) {
